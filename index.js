    ...
    if (message.content) {
        ...
        if (args[0].toLowerCase() === defaults.triggers.newPoll) {
            ...
            if (
                ...
                message.channel.send(`${message.author.toString()}, here is your poll.`, { embed });

            } else {
                console.error("Message format was invalid.");
                message.channel.send(`Poll requests must at minimum include a title (in "double quotes") and a set of options (in [square brackets], separated by commas). For example, try \`${defaults.triggers.newPoll} "What is your favorite sport?" [basketball, soccer, baseball]\`.`);
            }

        } else if (args[0].toLowerCase() == defaults.triggers.vote) {
            args.shift();

            var activePollsInServer = [],
                voteResponse;
            polls.forEach(poll => {
